...
Ruuter.internal routes use `/ljvis/erru/nu/`. These are transport DTOs, not SOAP endpoints. XML token whitespace must be normalized before building a DTO. Inbound XML reaches `inbound-request` through `docker/erru-xml-adapter` (asynchronous ERRU 3.5 XML over HTTP, see [the architecture document](../architecture/erru-async-xml.md)): it validates XML against the supplied ERRU 3.5 schemas, builds the flattened DTO and turns the returned ACK JSON into `NotifyUnfitness_Acknowledgement` XML. A production ingress must also authenticate machine calls and supply the authenticated peer; neither the adapter nor an OAuth ingress does this yet. Do not expose these routes publicly. User cookies do not establish transport identity.
...
`POST inbound-request` accepts the existing flattened request. `sentAt` is required (UTC seconds, minimum 1753-01-01); absent `unfitStartDate` defaults to the first reception day in Europe/Tallinn. Optional `receivedAt` (UTC timestamp) is the transport's first-acceptance time: when present it is stored as `received_at` and used for that default, so queueing or retries cannot move the date; without it both use the processing time. The XML adapter always sends it. Empty optional DTO strings represent omitted XML attributes. Every supplied name/certificate block must be complete. Validation failures return HTTP 400 before persistence, with an ack-shaped body (`statusCode: InvalidData`); the XML adapter sends an `ErrorNotification` with `InvalidData`. It never substitutes the Hub-only ACK status `NotAvailable`.

The response contains the stable ACK Header fields, `businessCaseId`, `originatingAuthority` and `memberStates`. `acknowledgementType` is transport metadata and must not be serialized as an ERRU XML attribute. Optional empty strings must be omitted from XML. Duplicate reception returns the stored ACK, including the original identifiers. If the earlier attempt committed the snapshot but stopped before the in-app notification and audit event, the duplicate creates them first (legacy audit events are found via `erru/nu/audit-event-exists`; new events use `erru.record_inbound_audit` with a stable message key and transaction lock, while the notification is idempotent on its own). Preparing the ACK and recording the received/acknowledged snapshots is one transaction; a database failure rolls back the operation and must become a transport fault.

For inbound requests, the XML adapter stores and delivers the ACK through its outbox, enforcing the response deadline; expiry does not cancel local NU processing. See [XML recovery](erru-xml-recovery.md). Outbound NU calls still use the synchronous contract below. Returning an ACK from Ruuter does not establish that the remote peer received its bytes.
