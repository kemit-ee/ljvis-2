...
client_data:
  member_class: GOV
  member_code: "70006317"       # LJVIS liikme registreerimisnumber
  subsystem_code: ljvis

security_server:
  url: "https://urien.ml.ee:5500/"
  keystore_path: /app/ssl/xtr-client.p12
  keystore_password_env: XTR_KEYSTORE_PASSWORD   # loetakse keskkonna muutujast

wsdl_watch_dir: /app/wsdl      # WSDL folder-drop (vt allpool)
...
### Konfiguratsioon otsimise järjekord

1. `--config <tee>` käsurea argument
2. `XTR_CONFIG` keskkonna muutuja
3. `./xtr.yaml` või `./xtr.yml`
4. Sisseehitatud vaikeväärtused
...
XTR toetab automaatset DSL-ide genereerimist WSDL-failidest käivitumisel. Kui `wsdl_watch_dir` on seadistatud, parsib XTR kõik `<group>/*.wsdl` failid ja kirjutab vastavad DSL-id `dsl_path`-i.

LJVIS-is asuvad WSDL-id kaustas `wsdl/ar/ariregister.wsdl`. Genereeritud DSL-id on märgistatud reaga:
```
