...
| Field | Description |
|---|---|
| `event_id` | ULID (26-char base32 Crockford). Time-ordered, sortable, globally unique. Primary key. Generated by `audit.generate_ulid()` inside the INSERT transaction; application callers leave event_id empty. Allocation holds the chain-tip lock and increments beyond the previous ULID when necessary, preserving insertion order under concurrency and within one millisecond. Do not preallocate IDs in application code. |
