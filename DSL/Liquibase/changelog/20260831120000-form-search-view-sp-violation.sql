...
-- changeset ljvis:20260831120000 ignore:true
--
-- has_violation SP-alamvormidele: KORRAS ja bare HOIATUS ei loe rikkumiseks;
-- HOIATUS + lisameede ning ALUSTATI loevad. Parandab ka varasema vea, kus iga
-- kinnitatud KORRAS-vorm luges rikkumiseks (result_type <> 'ok' — frontend ei
-- salvesta kunagi 'ok'-i). Muu view muutmata (CREATE OR REPLACE, tram-eristus säilib).

--
-- LJVIS2-9 (Otsingute tegemine) — cross-entity form search VIEW.
--
-- Projects the latest snapshot of every form type onto one common column set so
-- a single query can search across all control forms. "Current state" of each
-- form = DISTINCT ON (<key>) ORDER BY <key>, created_at DESC (INSERT-only
-- snapshot model). Rows with status = 'deleted' are excluded (soft-deleted
-- koondvorm and its sub-forms are hidden from all searches). Sub-forms
-- (sp_driver, sp_teammate, vehicle/trailer_technical, adr, kv) carry no business
-- data of their own — vehicle / company / driver / location / date are inherited
-- from the latest non-deleted parent compound_form via INNER JOIN.
--
-- has_violation rule per type:
--   foreign_violation : sanction_code <> 'KORRAS' OR violations <> '[]'
--   labour_inspection : violations <> '[]'
--   good_repute       : fitness_status = 'unfit'
--   sp_driver/sp_teammate/vehicle_technical/trailer_technical/adr : result_type <> 'ok'
--   kv                : false (no result field)
--   compound          : false (parent-level aggregation across sub-forms is phase 2)
