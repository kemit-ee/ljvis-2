...
-- changeset ljvis:20260829100000 ignore:true
--
-- Fix forms.form_search (20260806100000): driver_search for compound_form and
-- its sub-forms (sp_driver, sp_teammate, vehicle_technical, trailer_technical,
-- adr, kv) was reading compound_form.drivers JSONB via snake_case keys
-- (personal_code_ee/personal_code_foreign/first_name/last_name), but every
-- writer of that column (CompoundFormEditCard/CompoundFormCreatePage, and the
-- GDPR masking + ownership checks in Ruuter's citizen/forms/compound.yml) uses
-- camelCase (personalCodeEe/personalCodeForeign/firstName/lastName). The
-- mismatch meant driver_search was always NULL for these 7 form types, so a
-- citizen's personal_code (füüsiline isik / citizen-self search) could never
-- match a compound_form-derived row — company_reg_code search was unaffected.
-- foreign_violation/labour_inspection/good_repute build driver_search from
-- plain table columns, not JSON, so they were never affected by this bug.
...
COMMENT ON VIEW forms.form_search IS 'LJVIS2-9 cross-entity form search. One row per latest, non-deleted snapshot of each form type (10 types incl. sub-forms). Sub-form business fields inherited from latest non-deleted parent compound_form. Backing view for RESQL control-forms/search/search. driver_search for compound-derived types reads camelCase JSON keys (20260829100000).';
