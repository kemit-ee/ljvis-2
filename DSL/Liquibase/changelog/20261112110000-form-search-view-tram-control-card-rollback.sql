...
-- Rollback: taasta forms.form_search 20261101130000 kujule (tram_compound/tram_driver harud).
--
-- VR-kontrollkaart spetsiifilised otsingufiltrid: reportingCountryCode ja sanctionCode.
-- RS ettepanek (18.05.2026): VR vormiotsingus puuduvad spetsiifilised filtrid.
-- Lisame forms.form_search VIEW-le kaks uut veergu:
--   vr_reporting_country_code TEXT  – ainult foreign_violation ridadel, muudel NULL
--   vr_sanction_code          TEXT  – ainult foreign_violation ridadel, muudel NULL
-- VIEW uuesti kirjutamine kõigi UNION ALL harudega (kõik samad, ainult uued veerud lisatud).
--
...
COMMENT ON VIEW forms.form_search IS 'LJVIS2-9 cross-entity form search. One row per latest, non-deleted snapshot of each form type (10 types incl. sub-forms). Sub-form business fields inherited from latest non-deleted parent compound_form. Backing view for RESQL control-forms/search/search. vr_reporting_country_code and vr_sanction_code added 20261101 for VR-specific filters.';
