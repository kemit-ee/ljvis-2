...
BEGIN
    ...
    SELECT id INTO v_kliim_org_id FROM ljvis2.organisation WHERE code = 'KLI';
    SELECT id INTO v_trans_org_id FROM ljvis2.organisation WHERE code = 'TRA';
    ...
    SELECT id INTO v_liisa_user_id FROM ljvis2.user_account WHERE personal_code = '49501010011';
    SELECT id INTO v_peeter_user_id FROM ljvis2.user_account WHERE personal_code = '49602020022';
    ...
    IF NOT EXISTS (
        ...
        INSERT INTO ljvis2.user_account_state (user_account_id, status, created_at, created_by)
    ...
    FROM ljvis2.user_group_name_state ugns
    ...
    IF v_classifier_group_id IS NULL THEN
        ...
        INSERT INTO ljvis2.user_group_name_state (user_group_id, name, created_at, created_by)
        VALUES (v_classifier_group_id, 'Klassifikaatori haldurid', now() - INTERVAL '1 day', v_seed_by);
    END IF;

    SELECT id
    INTO v_link_id
    FROM ljvis2.user_group_organisation
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM ljvis2.user_group_organisation_state
    ...
    INSERT INTO ljvis2.permission (code, description, created_by)
    ...
    FOREACH v_permission_id IN ARRAY ARRAY[
        (SELECT id FROM ljvis2.permission WHERE code = 'classifier.list'),
        (SELECT id FROM ljvis2.permission WHERE code = 'classifier.read'),
        (SELECT id FROM ljvis2.permission WHERE code = 'classifier.edit'),
        (SELECT id FROM ljvis2.permission WHERE code = 'classifier_value.edit')
    ]
    LOOP
        SELECT id
        INTO v_link_id
        FROM ljvis2.user_group_permission
        WHERE user_group_id = v_classifier_group_id
        ...
        IF NOT EXISTS (
            SELECT 1
            FROM ljvis2.user_group_permission_state
    ...
    FROM ljvis2.user_account_user_group
    WHERE user_account_id = v_liisa_user_id
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM ljvis2.user_account_user_group_state
    ...
    IF NOT EXISTS (
        ...
        INSERT INTO ljvis2.user_group_latest (
            ...
            false,
            jsonb_build_array(
                jsonb_build_object('id', (SELECT id FROM ljvis2.permission WHERE code = 'classifier.list'), 'code', 'classifier.list'),
                jsonb_build_object('id', (SELECT id FROM ljvis2.permission WHERE code = 'classifier.read'), 'code', 'classifier.read'),
                jsonb_build_object('id', (SELECT id FROM ljvis2.permission WHERE code = 'classifier.edit'), 'code', 'classifier.edit'),
                jsonb_build_object('id', (SELECT id FROM ljvis2.permission WHERE code = 'classifier_value.edit'), 'code', 'classifier_value.edit')
            ),
            v_seed_by
        );
    END IF;
    ...
    SELECT id INTO v_classifier_id FROM ljvis2.classifier WHERE code = 'RIIK';
    IF NOT EXISTS (
        SELECT 1 FROM ljvis2.classifier_name_state WHERE classifier_id = v_classifier_id AND name = 'Riik'
    ) THEN
        INSERT INTO ljvis2.classifier_name_state (classifier_id, name, description, created_by)
        VALUES (v_classifier_id, 'Riik', 'Riikide klassifikaator', v_admin_user_id);
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM ljvis2.classifier_latest WHERE classifier_id = v_classifier_id AND created_by = v_admin_user_id
    ) THEN
        INSERT INTO ljvis2.classifier_latest (classifier_id, code, name, description, created_by)
        VALUES (v_classifier_id, 'RIIK', 'Riik', 'Riikide klassifikaator', v_admin_user_id);
    END IF;

    SELECT id INTO v_classifier_id FROM ljvis2.classifier WHERE code = 'DOKL';
    IF NOT EXISTS (
        SELECT 1 FROM ljvis2.classifier_name_state WHERE classifier_id = v_classifier_id AND name = 'Dokumendi liik'
    ) THEN
        INSERT INTO ljvis2.classifier_name_state (classifier_id, name, description, created_by)
    ...
    SELECT id INTO v_classifier_id FROM ljvis2.classifier WHERE code = 'SUGU';
    IF NOT EXISTS (
        SELECT 1 FROM ljvis2.classifier_name_state WHERE classifier_id = v_classifier_id AND name = 'Sugu'
    ) THEN
        INSERT INTO ljvis2.classifier_name_state (classifier_id, name, description, created_by)
    ...
    SELECT id INTO v_classifier_value_id FROM ljvis2.classifier_value WHERE classifier_id = v_classifier_id AND code = 'EE';
    IF NOT EXISTS (
        SELECT 1 FROM ljvis2.classifier_value_validity_state WHERE classifier_value_id = v_classifier_value_id AND valid_from = DATE '2020-01-01' AND valid_until IS NULL
    ) THEN
        INSERT INTO ljvis2.classifier_value_validity_state (classifier_value_id, valid_from, valid_until, created_by)
        VALUES (v_classifier_value_id, DATE '2020-01-01', NULL, v_admin_user_id);
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM ljvis2.classifier_value_latest WHERE classifier_value_id = v_classifier_value_id AND created_by = v_admin_user_id
    ) THEN
        INSERT INTO ljvis2.classifier_value_latest (classifier_value_id, classifier_id, classifier_code, code, name, valid_from, valid_until, is_valid, created_by)
    ...
    ON CONFLICT (classifier_id, code) DO NOTHING;
    SELECT id INTO v_classifier_value_id FROM ljvis2.classifier_value WHERE classifier_id = v_classifier_id AND code = 'FI';
    IF NOT EXISTS (
        SELECT 1 FROM ljvis2.classifier_value_validity_state WHERE classifier_value_id = v_classifier_value_id AND valid_from = DATE '2020-01-01' AND valid_until IS NULL
    ) THEN
        INSERT INTO ljvis2.classifier_value_validity_state (classifier_value_id, valid_from, valid_until, created_by)
    ...
    ON CONFLICT (classifier_id, code) DO NOTHING;
    SELECT id INTO v_classifier_value_id FROM ljvis2.classifier_value WHERE classifier_id = v_classifier_id AND code = 'LV';
    IF NOT EXISTS (
        SELECT 1 FROM ljvis2.classifier_value_validity_state WHERE classifier_value_id = v_classifier_value_id AND valid_from = DATE '2020-01-01' AND valid_until = DATE '2025-12-31'
    ) THEN
        INSERT INTO ljvis2.classifier_value_validity_state (classifier_value_id, valid_from, valid_until, created_by)
    ...
    ON CONFLICT (classifier_id, code) DO NOTHING;
    SELECT id INTO v_classifier_value_id FROM ljvis2.classifier_value WHERE classifier_id = v_classifier_id AND code = 'TAOTLUS';
    IF NOT EXISTS (
        SELECT 1 FROM ljvis2.classifier_value_validity_state WHERE classifier_value_id = v_classifier_value_id AND valid_from = DATE '2024-01-01' AND valid_until IS NULL
    ) THEN
        INSERT INTO ljvis2.classifier_value_validity_state (classifier_value_id, valid_from, valid_until, created_by)
    ...
    ON CONFLICT (classifier_id, code) DO NOTHING;
    SELECT id INTO v_classifier_value_id FROM ljvis2.classifier_value WHERE classifier_id = v_classifier_id AND code = 'LUBA';
    IF NOT EXISTS (
        SELECT 1 FROM ljvis2.classifier_value_validity_state WHERE classifier_value_id = v_classifier_value_id AND valid_from = DATE '2024-01-01' AND valid_until IS NULL
    ) THEN
        INSERT INTO ljvis2.classifier_value_validity_state (classifier_value_id, valid_from, valid_until, created_by)
    ...
    ON CONFLICT (classifier_id, code) DO NOTHING;
    SELECT id INTO v_classifier_value_id FROM ljvis2.classifier_value WHERE classifier_id = v_classifier_id AND code = 'M';
    IF NOT EXISTS (
        SELECT 1 FROM ljvis2.classifier_value_validity_state WHERE classifier_value_id = v_classifier_value_id AND valid_from = DATE '2020-01-01' AND valid_until IS NULL
    ) THEN
        INSERT INTO ljvis2.classifier_value_validity_state (classifier_value_id, valid_from, valid_until, created_by)
    ...
    ON CONFLICT (classifier_id, code) DO NOTHING;
    SELECT id INTO v_classifier_value_id FROM ljvis2.classifier_value WHERE classifier_id = v_classifier_id AND code = 'N';
    IF NOT EXISTS (
        SELECT 1 FROM ljvis2.classifier_value_validity_state WHERE classifier_value_id = v_classifier_value_id AND valid_from = DATE '2020-01-01' AND valid_until IS NULL
    ) THEN
        INSERT INTO ljvis2.classifier_value_validity_state (classifier_value_id, valid_from, valid_until, created_by)
    ...
    ON CONFLICT (classifier_id, code) DO NOTHING;
    SELECT id INTO v_classifier_value_id FROM ljvis2.classifier_value WHERE classifier_id = v_classifier_id AND code = 'X';
    IF NOT EXISTS (
        SELECT 1 FROM ljvis2.classifier_value_validity_state WHERE classifier_value_id = v_classifier_value_id AND valid_from = DATE '2020-01-01' AND valid_until IS NULL
    ) THEN
        INSERT INTO ljvis2.classifier_value_validity_state (classifier_value_id, valid_from, valid_until, created_by)
