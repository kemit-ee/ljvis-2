...
INSERT INTO users.permission (code, description, created_by) VALUES ('xtee.query',                      'X-tee päringute tegemine (kõik andmeallikad)', 'ljvis2') ON CONFLICT (code) DO NOTHING;
...
SELECT
    ...
    ARRAY['user_group.list.admin','user_group.read.admin','user_group.read.local','user_group.create','user_group.update','user_group.list_users.admin','user_group.search_eligible_users','user_group.add_user','user_group.remove_user','user.list.admin','user.read.admin','user.edit.admin','organisation.list','permission.list','classifier.read','labour_inspection_form.write','labour_inspection_form.read','control_form.view_unpublished','control_form.delete','control_form.edit_locked','compound_form.write','compound_form.read','vehicle_technical_form.write','vehicle_technical_form.read','trailer_technical_form.write','trailer_technical_form.read','adr_form.write','adr_form.read','transport_interruption_form.write','transport_interruption_form.read','good_repute_form.write','good_repute_form.read','sp_driver_form.write','sp_driver_form.read','sp_teammate_form.write','sp_teammate_form.read','xtee.query.rahvastikuregister','xtee.query','ctud.read','ctud.create','ctud.send','cgr.read','cgr.create','cgr.send','rsi.read','rsi.create','rsi.send','ncr.read','ncr.create','ncr.respond','ncr.send','ncr.list','risk_report.list','notification.admin']::TEXT[],
    'ljvis2'
