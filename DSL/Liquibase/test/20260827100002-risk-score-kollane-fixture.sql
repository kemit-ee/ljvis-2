...
--
-- 90000005 "Riskiskoori Test AS Kollane" — 1 published control:
--   1x MSI(90) + 1x VSI(30) = 120 weighted; r=1 => R=120 => Kollane (101<=R<=200)
--
DO $$
DECLARE
  cf_key_5 BIGINT := 95000501;
BEGIN
  INSERT INTO forms.compound_form (
    ...
    '[]'::jsonb, '90000005', 'Riskiskoori Test AS Kollane', '[]'::jsonb, now() - INTERVAL '15 days', 'system'
  );
  ...
  INSERT INTO forms.sp_driver_form (
    ...
    95100501, cf_key_5, 'sp-2026-95100501/1', 1, 'published', 'active',
