...
-- changeset ljvis:20260814100000 ignore:true splitStatements:false
--
-- Test/dev-only seed for the ERRU RSI module (LJVIS2-146 / -147 / -148 / -149).
--
-- NOTE: COUNTRY is already seeded by 20260526100001-seed-data.sql — this file must NOT
-- re-insert it. rsi_from / rsi_to / vehicle_registration_country / driver_licence_country /
-- identification_details.address.country all reference COUNTRY.
-- TECHNICAL_CHECK (level 1, real production values CAA_0..CAA_11) already exists in the
-- PRODUCTION changelog (20260803150000-initial-technical-check-form.sql) — reused verbatim
-- for rsi_message.checked_items[].partCode, CAA_10 excluded per LJVIS2-147/-148.
--
-- RSI_REQUEST_SOURCE / RSI_REQUEST_PURPOSE mirror the CGR/CTUD per-family classifiers
-- (globalRequestSourceType / globalRequestPurposeType are the same ERRU-wide enum, kept
-- separate per family for consistency with the existing CTUD_*/CGR_* naming convention —
-- see 20260812100000-erru-cgr-classifier-seed.sql comment).
-- RSI_RESPONSE_STATUS values come from RoadSideInspection_Types.xsd rsiResponseStatusType
-- (OK/NotFound — RSI has no Timeout/NotAvailable, unlike CGR/CTUD, because the response
-- is a separate correlated message, not a Hub-mediated synchronous exchange).
-- RSI_VEHICLE_CATEGORY values are the full rsiVehicleCategoryType enum (RoadSideInspection_
-- Types.xsd) — wire values used directly as codes, Estonian names are the general EU
-- type-approval category labels since no per-code label is given in LJVIS2-147/-148.
