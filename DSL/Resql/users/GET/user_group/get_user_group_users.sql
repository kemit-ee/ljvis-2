...
WHERE
  ...
  AND (
    COALESCE(:is_local_admin, 'false') != 'true'
        OR (SELECT d.organisation_id FROM ljvis2.user_account_data_state d WHERE d.user_account_id = ua.id ORDER BY d.created_at DESC LIMIT 1) = :user_organisation_id::BIGINT
    )
  AND (
    COALESCE(:search, '') = ''
