/*
description: Read one CGR request by its logical key (LJVIS2-138). Returns the LATEST snapshot only —
  current state is always the most recent row for cgr_request_key. Returns zero rows when the key does
  not exist, which the caller maps to 404. Columns are emitted in snake_case and serialised to camelCase
  by Resql; member_states is cast to text so the JSONB document is passed through verbatim.
namespace: erru
params:
  id:
    type: integer
