/*
description: Paginated, filtered list of CTUD requests, both directions in one table (LJVIS2-145). The
  `latest` CTE reduces the append-only table to exactly one row per request (the newest snapshot) before
  any filtering or paging, so a request with ten snapshots still occupies one list row. All filters are
  optional and AND-combined, except the three search-criteria fields (undertaking name, community licence
  number, vehicle registration number) which are OR-combined with each other and then AND-combined with
  the rest. Sorting is whitelisted; total is returned via COUNT(*) OVER () so the caller needs only one
  round trip.
namespace: erru
params:
  businessCaseId:
