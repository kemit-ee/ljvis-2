/*
description: 'Record an INCOMING CTUD request arriving from another member state (LJVIS2-144). Appends
  the first snapshot with status ''received'' and a fresh logical key. Idempotent: ON CONFLICT on the
  partial unique index uq_ctud_inbound_technical_id means a redelivered ERRU message yields zero rows
  instead of a duplicate request, so the caller can detect the replay and return the previously produced
  response. business_case_id is stored verbatim as sent by the peer, and sent_at comes from the message
  header rather than the clock.'
namespace: erru
params:
  technicalId:
    type: string
