/*
description: 'Apply an RSI lifecycle state transition (LJVIS2-148). Appends exactly one new snapshot iff
  (current status, new status, direction) is in the allowed-transition whitelist; otherwise yields zero
  rows and nothing is written — the caller maps that to 422 invalid_transition. RSI differs from CGR in
  three critical ways: (1) error is a TERMINAL outgoing state — there is no error→sent retry path; a failed
  message must be discarded and a brand-new one composed. (2) RSI is asynchronous — after ''sent'' the
  response arrives later via a separate inbound-response call, so there is a distinct ''responded'' status
  in the outgoing chain. (3) There is no resend (no sent→sent); every send attempt must start from ''initiated''.
  Columns not driven by the transition (vehicle data, driver data, inspection data, identification_details,
  checked_items) are carried forward verbatim from the latest snapshot. response_status_code/response_status_message
  are set only on the responded/answered transitions.'
namespace: erru
params:
  key:
    type: integer
...
WITH latest AS (
  ...
  SELECT
    ...
    :newStatus,
    l.business_case_id,
    -- technical_id: an explicit value from the caller always wins (Ruuter generates it
    -- BEFORE calling XTR so the same id is stored here and sent in the SOAP message).
    -- Falls back to a fresh UUID on each outgoing 'sent' attempt if the caller omitted
    -- it. For all other transitions (responded, answered, error) carry the existing value.
    CASE
      ...
      ELSE l.technical_id
    END,
    -- workflow_id: an explicit value from the caller always wins, else generated once on
    -- the first outgoing 'sent', preserved on all subsequent snapshots including the
    -- asynchronous 'responded'. For incoming messages the sender provides the workflow_id
    -- from the envelope; it is preserved unchanged.
    CASE
