/*
description: Update the X-tee block fields (extraordinary_inspection_date, enforcement_decision, proceeding_closure_basis)
  IN PLACE on the latest snapshot row. Does not append a new snapshot and does not bump version — the
  sub-form number's /V suffix is intentionally unaffected (LJVIS2-72 §4). Caller (Ruuter .guard) must
  already have verified control_form.edit_locked and that the latest snapshot status is confirmed.
