...
params:
  ...
  trace_id:
    type: string
    required: false
    description: W3C tracecontext trace id (32-hex) from the originating request traceparent header. NULL
      if absent.
  span_id:
    type: string
    required: false
    description: W3C tracecontext span id (16-hex) from the originating request traceparent header. NULL
      if absent.
returns:
- name: event_id
  type: string
