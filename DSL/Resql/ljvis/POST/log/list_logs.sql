...
params:
  ...
  organisation_id:
    type: string
    required: false
    description: When set, restrict results to audit events whose actor belonged to this organisation
      (audit.read.local scope). Empty = no restriction (audit.read, all organisations).
returns:
- name: event_id
  type: string
  nullable: true
